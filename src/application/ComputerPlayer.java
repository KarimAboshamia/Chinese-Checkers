...
public class ComputerPlayer {
	...
	public int calculateH(List<Node> pc, List<Node> human) {
		int total = 0;
		for(int i = 0; i < pc.size(); i++) {
			total += (17 - pc.get(i).getX()) + 4 * human.get(i).getX() + Math.abs(5 - human.get(i).getY()) + Math.abs(13 - human.get(i).getZ());	
		}
		System.out.println("PC total " + total);
		return total;
	}
	
	public int calc(List<Node> humanAlwaysUpdating, List<Node> staticPC) {
		int total = 0;
		for(int i = 0; i < humanAlwaysUpdating.size(); i++) {
			total += (17 - staticPC.get(i).getX()) + 4 * humanAlwaysUpdating.get(i).getX() + Math.abs(5 - humanAlwaysUpdating.get(i).getY()) + Math.abs(13 - humanAlwaysUpdating.get(i).getZ());	
		}
		System.out.println("Human total " + total);
		return total;
	}
	...
	public void allMoves(HashMap<String, Node> MapBord, List<Node> pcNodes) {
		...
		for (Node nod : pcNodes) {
			...
			if (checkValid(nod.getX() + 1, nod.getY() - 1, nod.getZ())) {
				...
				int heur = calculateH(tmpPc, currentHumanNodes);
				heuristicLevelOne.add(heur);
	...
	public void moveLevel2(HashMap<String, Node> MapBord, List<Node> pcNodes) {
		...
		for (Node nod : pcNodes) {
			...
			if (checkValid(nod.getX() + 1, nod.getY() - 1, nod.getZ())) {
				...
				int heur = calc(tmpPc, modifiedPC);
				heuristicLevelTwo.add(heur);
	...
	public void jump2(Node nod, HashMap<String, Node> tmpState, List<Node> tmpPc, int counter,
		...
		if (checkValid(nod.getX() + 2, nod.getY() - 2, nod.getZ())
				&& checkValid2(nod.getX() + 1, nod.getY() - 1, nod.getZ()) && !(visitedJumps.contains(key5)))
		{
			String keySearch1 = Integer.toString(nod.getX()) + "," + Integer.toString(nod.getY()) + ","
					+ Integer.toString(nod.getZ());
			String keySearch2 = Integer.toString(nod.getX() + 2) + "," + Integer.toString(nod.getY() - 2) + ","
